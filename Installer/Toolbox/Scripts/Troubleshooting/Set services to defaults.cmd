...
set "servicesPath=%windir%\EBDesktop\6. Advanced Configuration\Services"
if not exist "%servicesPath%" (
	echo Services in EB folder not found, can't continue.
	if "%*"=="" pause
	exit /b 1
)
...
echo This will reset the configuration of services in the EB folder.
...
:main
%title% "Enabling services in the EB folder... This might take a while."
for /f "usebackq tokens=*" %%a in (`dir /b /s "%windir%\EBDesktop\6. Advanced Configuration\Services" ^| find "(default)"`) do (
	call :run "%%a"
	start /min /high /wait cmd /c "%%a" /silent
)

set "ebOther=%windir%\EBModules\Other"
set "winServices=%ebOther%\winServices.reg"
set "ebServices=%ebOther%\ebServices.reg"
if exist "%winServices%" (
	if exist "%ebServices%" call :fullRestore
)
...
echo 2) Restore a full services backup of the default EB services
echo 3) Nothing
echo]
choice /c:123 /n /m "Choose a number: [1/2/3] "
if "%errorlevel%"=="1" reg import "%winServices%" > nul
if "%errorlevel%"=="2" reg import "%ebServices%" > nul
exit /b
