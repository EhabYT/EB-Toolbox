...
namespace EBToolbox.Utils
{
    public static class ProfileSerializing
    {
        ...
        {
            FileInfo[] profileFile = new DirectoryInfo($"{Environment.GetEnvironmentVariable("windir")}\\EBModules\\Toolbox\\Profiles\\").GetFiles();
            ...
            File.WriteAllText($"{Environment.GetEnvironmentVariable("windir")}\\EBModules\\Toolbox\\Profiles\\{profileName}.json", jsonString);
            return DeserializeProfile($"{Environment.GetEnvironmentVariable("windir")}\\EBModules\\Toolbox\\Profiles\\{profileName}.json");
        }
