...
namespace EBToolbox.Services.ConfigurationServices
{
    ...
    {
        private const string EB_STORE_KEY_NAME = @"HKLM\SOFTWARE\EBOS\Services\Location";
        ...
        public void Disable()
        {
            ...
            RegistryHelper.SetValue(EB_STORE_KEY_NAME, "path", @$"{Environment.GetEnvironmentVariable("windir")}\EBDesktop\3. General Configuration\Location\Disable Location (default).cmd");

            CommandPromptHelper.RunCommand("sc stop lfscv");
            CommandPromptHelper.RunCommand("sc stop MapsBroker");
            CommandPromptHelper.RunCommand("call \"%windir%\\EBModules\\Scripts\\settingsPages.cmd\" /hide privacy-location");
            CommandPromptHelper.RunCommand("call \"%windir%\\EBModules\\Scripts\\settingsPages.cmd\" /hide findmydevice");
        ...
        public void Enable()
        {
            ...
            RegistryHelper.SetValue(EB_STORE_KEY_NAME, "path", @$"{Environment.GetEnvironmentVariable("windir")}\EBDesktop\3. General Configuration\Location\Enable Location.cmd");

            CommandPromptHelper.RunCommand("sc start lfscv");
            CommandPromptHelper.RunCommand("sc start MapsBroker");
            CommandPromptHelper.RunCommand("call \"%windir%\\EBModules\\Scripts\\settingsPages.cmd\" /unhide privacy-location");
            CommandPromptHelper.RunCommand("call \"%windir%\\EBModules\\Scripts\\settingsPages.cmd\" /unhide findmydevice");
