...
namespace EBToolbox.Services.ConfigurationServices
{
    ...
    {
        private const string EB_STORE_KEY_NAME = @"HKLM\SOFTWARE\EBOS\Services\RecentItems";
        ...
        public void Disable()
        {
            ...
            RegistryHelper.SetValue(EB_STORE_KEY_NAME, "path", @$"{Environment.GetEnvironmentVariable("windir")}\EBDesktop\4. Interface Tweaks\Unlock Recent Items\Disable Recent Items (default).cmd");

            CommandPromptHelper.RunCommand(@"call ""%windir%\EBModules\Scripts\settingsPages.cmd"" /hide privacy-general");
        ...
        public void Enable()
        {
            ...
            RegistryHelper.SetValue(EB_STORE_KEY_NAME, "path", @$"{Environment.GetEnvironmentVariable("windir")}\EBDesktop\4. Interface Tweaks\Unlock Recent Items\Enable Recent Items.cmd");

            CommandPromptHelper.RunCommand(@"call ""%windir%\EBModules\Scripts\settingsPages.cmd"" /unhide privacy-general");
            CommandPromptHelper.RestartExplorer();
        }
